...
pub async fn get_license(
    ...
    let row = sqlx::query!(
        "SELECT trial_ends_at, access_expires_at, license_type FROM public.users WHERE id = $1",
        user_id
    )
    ...
    // Calcul des jours restants et de la fenêtre d'alerte
    let days_until_expiry: Option<i64> = if status == "expired" {
        None
    } else {
        ...
        // Lifetime (~100 ans) : jamais d'alerte
        if days_remaining > 3650 {
            None
        } else {
            // Seuil depuis la durée du dernier jeton
            let last_duration = if is_active {
                sqlx::query_scalar!(
            ...
            let threshold: i64 = match last_duration {
                Some(d) if d <= 30 => 7,
                Some(d) if d <= 95 => 15,
