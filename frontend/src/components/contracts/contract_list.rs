...
// Version COMPLÈTE — affiché dans l'onglet Contrats
...
#[component]
pub fn ContractList(vehicle_id: ReadSignal<Option<Uuid>>) -> impl IntoView {
                        ...
                        return view! {
                            <div class="bg-white rounded-xl border border-dashed border-gray-200 p-12 text-center">
                                <p class="text-sm text-gray-400 italic">
                                    "Aucun contrat enregistré pour ce véhicule."
                                </p>
                            </div>
                        }.into_view();
                    }
                            ...
                            {if !d.insurance.is_empty() { view! {
                                <div class="flex flex-col gap-3">
                                    ...
                                    {d.insurance.into_iter().map(|c| view! {
                                        <ContractInsuranceCard contract=c />
                                    }).collect_view()}
                                </div>
                            }.into_view() } else { view! { <div /> }.into_view() }}
                        </div>
    ...
    view! {
        <div class="bg-white rounded-xl border border-gray-100 p-5 space-y-4 shadow-sm">
            <div class="flex items-center justify-between">
                ...
                <span class=format!("text-xs font-medium px-2.5 py-1 rounded-full {}", badge_color)>
                    {badge_label}
                </span>
            </div>
            ...
            <div>
                ...
                <div class="w-full bg-gray-100 rounded-full h-2.5">
                    <div
                        class=format!("h-2.5 rounded-full transition-all {}", bar_color)
            ...
            <div class="grid grid-cols-3 gap-3">
                ...
                <div class=format!(
                    ...
                    if contract.overage_risk { "bg-amber-50" } else { "bg-gray-50" }
                )>
                    <p class="text-xs text-gray-400 mb-1">"Projection"</p>
                    <p class=format!(
                        "text-sm font-bold {}",
                        if contract.overage_risk { "text-amber-600" } else { "text-gray-800" }
                    )>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center px-4">
            <div class="bg-white rounded-2xl shadow-2xl border border-gray-100 w-full max-w-md p-8 space-y-6">
                <div class="flex items-center justify-between">
                    ...
                    <button
                        on:click=move |_| on_close.call(())
                        class="text-gray-400 hover:text-gray-600 text-xl font-light transition duration-150"
                    >
                        "✕"
                    </button>
                </div>
