...
- **Migration 022 — `vehicle_access` désynchronisé entre prod et dépôt versionné** (2026-09-16) : la prod avait un trigger `trg_auto_grant_owner` (accorde automatiquement le rôle `owner` à la création d'un véhicule) et deux colonnes d'audit (`granted_by`, `granted_at`) ajoutés manuellement à un moment non documenté, absents de toute migration et de la base de dev locale. Formalisés dans `022_vehicle_access_audit_and_trigger.sql` — sans effet sur la prod (déjà dans cet état), nécessaire pour tout autre environnement (dev local, CI). Repéré en réappliquant `seed_appstore_review.sql` sur la prod : l'insertion explicite des accès `owner` du seed n'avait inséré qu'1 ligne sur 6 attendues (les 5 autres déjà créées par le trigger). Corrige au passage une note erronée de `CLAUDE.md` qui affirmait qu'aucun trigger de ce type n'existait (vérifié uniquement sur la base locale à l'époque).

### Données de démo
- **`seed_appstore_review.sql` mis à jour et complété** (2026-09-16) — dates rafraîchies (décalage uniforme +105j pour préserver tous les scénarios relatifs : LOA "expire dans 28 jours", véhicule expiré, etc.) ; ajout de voyages planifiés et d'un carnet d'entretien complet (3 statuts prevu/devis/realise) sur le véhicule vitrine AR-001-AA. Un bug corrigé au passage : `contracts_insurance.km_start` d'AR-001-AA (20 000) était très inférieur au kilométrage réel à la date de début du contrat (~27 800), gonflant artificiellement la consommation annuelle apparente et déclenchant un faux `overage_risk`. Revérifié de bout en bout via l'API (contrats, `maintenance-status`, `usage-forecast`, `trips`) après correctif. Le script insère désormais aussi explicitement les accès `owner` (ceinture et bretelles avec le trigger de la migration 022, nécessaire pour tout environnement sans ce trigger).
...
- **Widget "Capacité kilométrique"** (ex-"Voyages planifiés" sur le dashboard) : renommé pour refléter son contenu (km disponibles, pas la liste des voyages) ; affiche désormais 3 tuiles (par jour / par semaine / par mois) au lieu d'un seul chiffre.
- **Distinction des libellés de projection** : le widget "Contrat actif" précise "(rythme actuel)" — calcul basé uniquement sur l'historique de conduite, sans les voyages planifiés — pour ne pas être confondu avec la date d'indisponibilité du widget "Capacité kilométrique", qui elle inclut les voyages planifiés ("voyages inclus").
- **Bouton d'accueil** : "Commencer gratuitement" → "Créer un compte" (plus explicite).
