...
-- maintenance_entries.status TEXT NOT NULL DEFAULT 'prevu' CHECK (prevu|devis|realise) — migration 021,
--                        seul 'realise' compte pour compute_maintenance_status (voir section Carnet d'entretien)
```
...
PATCH/DELETE /api/vehicles/:id/maintenance-entries/:entry_id  ← PATCH : mêmes champs que la création (hors pièces jointes)
...
Combine `daily_rate = km_consumed / days_elapsed` (même formule que `estimate_limit_date` dans `contracts_handler.rs`) avec les voyages planifiés actifs pour projeter jour par jour l'usage cumulé jusqu'à `end_date`. Retourne `km_per_day_available` (peut être négatif = dépassement déjà prévisible), `unavailable_from`/`unavailable_days`/`unavailable_until` (première date où le cumul atteindrait le plafond, jours restants, et date de fin du contrat correspondant), `recommended_daily_reduction_km`/`recommended_days_off` (voir ci-dessous), et `points` (échantillonnage hebdomadaire pour le graphique). Si LOA **et** assurance sont actifs simultanément, calcule les deux et retourne le plus restrictif (date d'indisponibilité la plus proche).

**`unavailable_days` n'est pas une durée d'indisponibilité temporaire** — c'est le nombre de jours entre `unavailable_from` et `unavailable_until` (= la date de fin du contrat). Au rythme actuel, le dépassement ne se résorbe jamais tout seul ; il dure jusqu'à l'échéance du contrat. Signalé confus par un utilisateur ("71 j" sans le "jusqu'au" associé) — le frontend affiche désormais explicitement `unavailable_until` ("Du X au Y (fin de contrat)") plutôt que la seule durée.

**Recommandations de récupération (v1.5.16)** : `overage_km` (calculé en interne, pas exposé) = dépassement projeté total à l'échéance, dérivé de la même simulation jour par jour que `unavailable_from` (pas une approximation linéaire séparée). `recommended_daily_reduction_km = ceil(overage_km / jours_restants)` (réduction à étaler sur **toute** la période restante, pas seulement sur la fenêtre de dépassement, toujours calculé et toujours réalisable en théorie) ; `recommended_days_off = ceil(overage_km / daily_rate)` — `None` si `daily_rate == 0` (aucun historique de conduite). Les deux ne sont calculés que si `unavailable_from.is_some()`.

**Bug corrigé (v1.5.17)** : `recommended_days_off` pouvait dépasser `days_remaining` — absurde ("ne pas rouler pendant plus de jours qu'il n'en reste"). Ça arrive quand le dépassement vient majoritairement des **voyages planifiés** plutôt que du rythme habituel (`daily_rate`) : arrêter complètement la conduite courante ne suffit alors pas à combler l'écart, puisque `daily_rate` ne représente que la conduite hors voyages. Repéré par un utilisateur via un cas réel (compte `apple.reviewer`, AR-001-AA — trajet domicile-travail quotidien + road trip, dépassement ≈ 4863 km alors que stopper la conduite habituelle sur les 107 jours restants n'en récupère que ≈ 3525). Fix : `recommended_days_off` n'est renvoyé (`Some`) que si `ceil(overage_km / daily_rate) <= days_remaining` — sinon `None`, et le frontend n'affiche alors que la ligne de réduction quotidienne (toujours valide, à répartir entre conduite courante et voyages).
...
**Modification d'une entrée existante (v1.5.21)** : `PATCH .../maintenance-entries/:entry_id` — même validation et même résolution de libellé que la création (logique extraite en fonction commune `resolve_entry_label`, appelée par les deux), types remplacés intégralement (`DELETE` puis ré-`INSERT` du pivot dans la même transaction plutôt qu'un diff, largement suffisant pour au plus `MAX_TYPES_PER_ENTRY` lignes). Cas d'usage : saisir rapidement une intervention prévue (ex. "Vidange" avec date/km approximatifs au moment du rendez-vous), puis revenir corriger le prix et la date exacts une fois la facture en main. **Les pièces jointes ne sont pas concernées** par ce endpoint — elles restent gérées exclusivement via `AttachmentsModal` (bouton 📎 sur la ligne d'historique), le champ fichier de `EntryModal` est masqué en mode édition (`is_edit`).

**Statut d'une entrée (v1.5.22, migration 021)** : `maintenance_entries.status` — `"prevu"` (intention/rappel, pas encore fait) / `"devis"` (chiffrage reçu, toujours pas fait) / `"realise"` (fait). Fonctionnellement binaire : seul `"realise"` compte pour `compute_maintenance_status` (le `JOIN` cherchant la dernière entrée par type filtre désormais `AND e.status = 'realise'`) — `"prevu"` et `"devis"` sont traités identiquement par ce calcul, la distinction entre les deux n'est qu'informative pour l'utilisateur (savoir s'il a déjà un chiffrage ou pas). **Volontairement pas déduit du coût** : un devis a lui aussi un coût renseigné mais n'est pas réalisé, donc `coût ≠ 0 → réalisé` serait faux — le statut est toujours choisi explicitement par l'utilisateur, jamais auto-inféré.
- Défaut base (`DEFAULT 'prevu'` sur la colonne, et `#[serde(default = "default_maintenance_entry_status")]` côté payloads communs) : `"prevu"`, sans risque de casser un usage existant puisqu'aucun client ne dépendait d'un statut avant cette version — la migration 021 backfill explicitement toutes les entrées déjà existantes à `"realise"` (elles représentent forcément du travail déjà fait, loggé après coup).
- Défaut formulaire (`EntryModal`, création) : **"Réalisé"** pré-sélectionné (pas "Prévu") — le cas d'usage dominant reste de noter une intervention juste après l'avoir fait faire ; l'utilisateur choisit explicitement "Prévu"/"Devis" pour le cas plus rare de la planification à l'avance.
- Bouton rapide "Marquer réalisé" sur les lignes d'historique en statut "Prévu"/"Devis" (visible seulement pour ces deux statuts) — renvoie un `PATCH` avec les données actuelles de l'entrée inchangées et `status: "realise"`, sans rouvrir le formulaire d'édition complet.
...
- **Layout responsive (v1.5.14)** : `flex flex-col md:flex-row` — empilés en pleine largeur sur mobile (le `<select>` catégorie compressait le `<select>` type à quelques caractères de large sur un petit écran en layout ligne fixe), alignés sur une ligne à partir de `md`. Les classes `input_class()` (qui inclut `w-full`) ne sont **pas** utilisées ici — le `w-full` entrerait en conflit avec les largeurs `flex-1`/`w-[9.5rem]` du layout desktop sans qu'on puisse fiablement prédire laquelle des deux classes gagne (même spécificité CSS, ordre dépendant de l'ordre de génération Tailwind, pas de l'ordre dans l'attribut `class`) ; les styles de champ sont dupliqués manuellement sans `w-full`.
...
- **Variante du même piège — `on:click` dans un enfant de `<Show>`** : un `on:click=move |_| { ...utilise une_variable_non_copy.clone()... }` placé directement dans les enfants d'un `<Show>` échoue aussi en `FnOnce` — la closure `move` du bouton capture (et déplace) la variable extérieure depuis l'environnement des enfants de `<Show>` (lui-même un `Fn` réévalué à chaque fois), donc le déplacement ne peut se produire qu'une fois. Fix (repéré sur le bouton "Modifier" d'une entrée d'entretien, `EntryModal`) : cloner la variable **avant** la closure `move`, dans un bloc `{ let x = x.clone(); move |_| { ...x.clone()... } }` — chaque exécution du bloc clone une copie fraîche depuis la variable extérieure encore intacte (jamais déplacée), au lieu de déplacer directement cette dernière. C'est exactement le même correctif que celui déjà utilisé sur le bouton "Supprimer" juste à côté (`let label = label.clone(); let id = e.id; move |_| ...`) — à reproduire systématiquement pour tout nouveau bouton ajouté à l'intérieur d'un `<Show>`.
- **Piège persistant même corrigé, si le bouton est dans un `<Show>` imbriqué dans un autre `<Show>`** : le clone-avant-`move` au niveau du `on:click` ne suffit plus — le `<Show>` extérieur doit *lui aussi* reconstruire le `<Show>` intérieur (donc son `ChildrenFn`) à chaque exécution, ce qui déplace la variable une deuxième fois, à un niveau plus haut. Plutôt que d'empiler un deuxième clone-avant-`move` à ce niveau, plus simple : remplacer le `<Show>` intérieur par une expression conditionnelle brute (`{condition.then(|| view! { ... })}`), qui n'introduit pas de `ChildrenFn` séparé — repéré sur le bouton "Marquer réalisé" (`EntryModal`/historique), visible seulement quand une entrée n'est pas encore "réalisée", imbriqué dans le `<Show when=can_manage>` englobant.
...
- **Scroll tactile dans un modal `position:fixed`** : signalé bloqué (aucun scroll du tout) sur une PWA Android installée (Samsung, mode standalone) — non reproduit malgré test avec geste tactile simulé (Chromium headless, émulation Galaxy S24, `Input.dispatchTouchEvent` via CDP), qui fonctionne. Fix défensif appliqué en attendant confirmation terrain : `overscroll-contain touch-pan-y` sur le conteneur scrollable (`Modal` dans `maintenance_list.rs`, `ViewerModal`) — pattern standard pour ce symptôme en `position:fixed`, sans effet de bord si ce n'était pas la cause réelle.
- **Root cause probable trouvée ensuite (capture d'écran desktop à l'appui)** : ce n'était pas (que) le scroll qui posait problème, mais le fait que le titre + bouton ✕ ET les boutons Annuler/Enregistrer faisaient partie du **contenu scrollable** de `Modal` — ils défilaient hors champ dès qu'on scrollait dans une liste longue (ex. checklist de types dans `EntryModal`), donnant l'impression que le modal était "bloqué, impossible d'en sortir" sans lien réel avec le scroll tactile en lui-même. **Fix (`Modal`/`ModalActions`, `maintenance_list.rs`)** : en-tête (`sticky top-0 z-10 bg-white`) et pied (`sticky bottom-0 bg-white border-t`, dans `ModalActions`) fixes, seul le contenu central défile. Vérifié via Chromium headless : après un scroll profond, ✕ reste visible en haut (y≈68px sur un viewport de 700px) et "Enregistrer" en bas (y≈594px), et cliquer sur ✕ depuis cet état ferme bien le modal.
- **Padding vertical du contenu (v1.5.18)** : le conteneur de contenu central de `Modal` n'avait aucun padding vertical propre (`class="px-8 space-y-6"`), collant directement sous le `pb-4` de l'en-tête sticky et contre le `pt-3` du pied sticky (`ModalActions`) — perceptible surtout sur `EntryModal` depuis le redesign du sélecteur en deux étapes (v1.5.13/v1.5.14), le contenu touchant visuellement le haut/bas du modal. Fix : `py-4` ajouté au conteneur (`class="px-8 py-4 space-y-6"`).
